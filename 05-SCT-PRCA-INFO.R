...
sumstats <- fread2(
  ...
  col.names = c("chr", "pos", "a1", "a0", "beta", "p", "INFO")
)
nrow(sumstats)  # 20,370,946
library(dplyr)
sumstats <- sumstats %>%
  mutate(chr = as.integer(chr), a0 = toupper(a0), a1 = toupper(a1),
         INFO = ifelse(INFO > 1 | INFO < 0.3, NA, INFO),
         beta = beta * INFO, p = p ** INFO, INFO = NULL) %>%
  filter(p < 0.1) %>%
  na.omit()
nrow(sumstats)  # 1,251,845
...
# 1,251,845 variants in summary statistics.
# 174,049 ambiguous SNPs have been removed.
# 1,024,366 variants have been matched; 3 were flipped and 459,673 were reversed.
info_snp <- bigsnpr::snp_match(sumstats, info_snp_UKBB, strand_flip = FALSE)
# 1,191,099 variants have been matched; 0 were flipped and 543,085 were reversed.
...
system.time(
  rds <- bigsnpr::snp_readBGEN(
    ...
    backingfile = "data/UKBB_PRCA_INFO",
...
library(bigsnpr)
ukbb <- snp_attach("data/UKBB_PRCA_INFO.rds")
G <- ukbb$genotypes
file.size(G$backingfile) / 1024^3  # 164 GB
...
system.time(
  multi_PRS <- snp_grid_PRS(
    ...
    n_thr_lpS = 50, backingfile = "data/UKBB_PRCA_INFO_scores", ncores = NCORES
  )
) # 49 min
...
length(ind <- which(new_beta != 0))  # 440,158
summary(new_beta)
#      Min.    1st Qu.     Median       Mean    3rd Qu.       Max.
# -1.8576570  0.0000000  0.0000000 -0.0001542  0.0000000  1.0340168
summary(new_beta[which(sign(new_beta * beta) < 0)])
#       Min.    1st Qu.     Median       Mean    3rd Qu.       Max.
# -3.365e-02 -1.425e-05  3.300e-07  3.104e-05  1.564e-05  7.040e-02

pred <- final_mod$intercept +
  big_prodVec(G, new_beta[ind], ind.row = ind.test, ind.col = ind)

AUCBoot(pred, y.sub[ind.test])
# Before: 71.7 [70.2-73.1] / 69.3 [68.7-70.0]
# After:  69.3 [67.8-70.8]
