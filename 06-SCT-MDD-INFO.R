...
sumstats <- fread2("sumstats_MDD.txt", fill = TRUE,
                   select = c("CHR", "BP", "A1", "A2", "OR", "P", "INFO"),
                   col.names = c("chr", "pos", "a1", "a0", "or", "p", "INFO"))
nrow(sumstats)  # 13,554,550
library(dplyr)
sumstats <- sumstats %>%
  mutate(chr = as.integer(chr),
         INFO = ifelse(INFO > 1 | INFO < 0.3, NA, INFO),
         beta = log(or) * INFO, or = NULL,
         p = p ** INFO, INFO = NULL) %>%
  filter(p < 0.13) %>%
  na.omit()
nrow(sumstats)  # 1,596,976
hist(sumstats$beta)
hist(sumstats$p)
...
# 1,596,976 variants in summary statistics.
# 223,274 ambiguous SNPs have been removed.
# 1,201,342 variants have been matched; 110 were flipped and 534,122 were reversed.
info_snp <- bigsnpr::snp_match(sumstats, info_snp_UKBB, strand_flip = FALSE)
# 1,421,490 variants have been matched; 0 were flipped and 642,461 were reversed.
...
# 1  500    0.2     0.3  14 1.325496 0.5676766
...
# 56.7 [55.5-57.9]
sum(lpval[ind.keep] > std_prs$thr.lp)  # 165,584 -> 68,981
...
# 59.9 [58.7-61.1]
sum(lpval[ind.keep] > max_prs$thr.lp)  # 222,912 -> 154,852

ggplot(grid2) +
  geom_point(aes(thr.lp, auc)) +
  facet_grid(thr.imp ~ thr.r2 + size) +
  scale_x_log10(limits = c(1, 3), breaks = c(1, 2, 4), minor_breaks = 1:10) +
  ylim(0.52, NA) +
