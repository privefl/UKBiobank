...
df0$has_cancer <- (rowSums(is.na(df_cancer)) < 18)
df0$has_breast_cancer <- (rowSums(df_cancer == 1002, na.rm = TRUE) >= 1)
head(df0)
stopifnot(all(which(df0$has_breast_cancer) %in% which(df0$has_cancer)))
